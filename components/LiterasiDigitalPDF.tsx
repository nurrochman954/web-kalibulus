...
const LiterasiDigitalPDF: React.FC<SinglePagePDFViewerProps> = ({
  pdfUrl,
  width = 800,
  height = 600,
  className = ''
  ...
  // Load PDF to get the number of pages
  useEffect(() => {
    ...
    const loadPdfInfo = async () => {
      try {
        ...
        setError(`Failed to load PDF file: ${error.message || 'Unknown error'}`);
        setIsLoading(false);
      }
    };
  ...
  useEffect(() => {
    const renderPage = async () => {
      ...
      try {
        ...
        // Calculate scale for single page view
        ...
        // Scale down canvas display for desired size
    ...
    return (
      <div className={`${className} flex flex-col justify-center items-center bg-gradient-to-br from-red-50 to-orange-50 border-2 border-red-200 rounded-2xl p-4 sm:p-8 shadow-lg mx-2 sm:mx-0`} style={{ height: containerHeight }}>
        ...
        <div className="text-xs sm:text-sm text-red-600 bg-white rounded-lg p-3 sm:p-4 shadow-md max-w-sm">
          <div className="font-medium mb-2">Ensure:</div>
          <ul className="space-y-1 text-left">
            <li className="flex items-center gap-2">
              <span className="w-2 h-2 bg-red-400 rounded-full flex-shrink-0"></span>
              <span>The PDF file is accessible</span>
            </li>
            <li className="flex items-center gap-2">
              <span className="w-2 h-2 bg-red-400 rounded-full flex-shrink-0"></span>
              <span>The PDF URL is correct</span>
            </li>
            <li className="flex items-center gap-2">
              <span className="w-2 h-2 bg-red-400 rounded-full flex-shrink-0"></span>
              <span>The server supports CORS</span>
            </li>
            <li className="flex items-center gap-2">
              <span className="w-2 h-2 bg-red-400 rounded-full flex-shrink-0"></span>
  ...
  return (
    ...
    >
      {isLoading && (
        <div className="flex flex-col justify-center items-center bg-gradient-to-br from-purple-50 to-indigo-100 rounded-2xl p-8 sm:p-12 shadow-lg mx-2 sm:mx-0" style={{ height: containerHeight, minWidth: isMobile ? '90vw' : 'auto' }}>
          <div className="relative">
            <div className="w-12 sm:w-16 h-12 sm:h-16 border-4 border-purple-200 border-t-purple-600 rounded-full animate-spin"></div>
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
              <div className="w-4 sm:w-6 h-4 sm:h-6 bg-purple-600 rounded-full animate-pulse"></div>
            </div>
          </div>
          <div className="text-center mt-6">
            <div className="text-lg sm:text-xl font-semibold text-purple-800 mb-2">Loading Digital Literacy PDF...</div>
            <div className="text-sm text-purple-600 opacity-80">Please wait a moment</div>
            <div className="mt-4 w-32 sm:w-48 h-2 bg-purple-200 rounded-full overflow-hidden">
      ...
      {!isLoading && numPages > 0 && (
        <>
            ...
            {isPageLoading && (
              <div style={{
                ...
                <div className="loading-spinner" style={{
                  ...
                  borderTop: '3px solid #8b5cf6', // purple-500
                  borderRadius: '50%',
          ...
          <div className="w-full max-w-4xl">
            <div className={`flex items-center justify-between bg-gradient-to-r from-purple-50 to-indigo-50 p-3 sm:p-4 rounded-2xl shadow-lg border border-purple-200 ${isMobile ? 'gap-2' : 'gap-4'}`}>
              ...
              <button
                ...
                className={`group flex items-center justify-center gap-1 px-3 py-3 sm:gap-2 sm:px-4 sm:py-3 rounded-xl font-medium text-xs sm:text-sm transition-all duration-300 transform flex-shrink-0 ${
                  currentPage === 1 
                    ...
                    : 'bg-gradient-to-r from-purple-500 to-indigo-600 hover:from-purple-600 hover:to-indigo-700 text-white shadow-lg hover:shadow-xl hover:scale-105 active:scale-95'
                }`}
              >
              ...
              {/* Page Selector */}
              <div className="flex items-center gap-1 sm:gap-3 bg-white rounded-xl px-2 sm:px-4 py-2 sm:py-3 shadow-md border border-purple-300 flex-shrink-0">
                <span className="text-xs sm:text-sm font-medium text-gray-700 hidden sm:inline">Page:</span>
                <select
                  value={currentPage}
                  onChange={(e) => goToPage(Number(e.target.value))}
                  className="bg-transparent border-none outline-none font-medium text-purple-700 cursor-pointer text-xs sm:text-sm min-w-[40px] sm:min-w-[60px] focus:ring-2 focus:ring-purple-400 rounded-lg px-1 sm:px-2 py-1"
                >
            ...
            {!isMobile && (
              <div className="flex flex-wrap gap-3 justify-center mt-4">
                <button 
                  onClick={() => goToPage(1)}
                  className="px-4 py-2 bg-white hover:bg-purple-50 text-purple-600 border border-purple-200 rounded-lg text-sm font-medium transition-all duration-200 hover:border-purple-400 hover:shadow-md"
                >
                  First Page
                </button>
                <button 
...
export default LiterasiDigitalPDF;
