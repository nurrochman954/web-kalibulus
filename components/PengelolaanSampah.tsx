...
interface PengelolaanSampahProps {
  pdfUrl: string;
  width?: number;
  height?: number;
  className?: string;
...
const PengelolaanSampah: React.FC<PengelolaanSampahProps> = ({
  pdfUrl,
  width = 800,
  height = 600,
  className = ''
  ...
  return (
    <div 
      ref={containerRef}
      className={`pengelolaan-sampah-container ${className}`} 
      style={{ 
        display: 'flex', 
    ...
    >
      {isLoading && (
        <div className="flex flex-col justify-center items-center bg-gradient-to-br from-green-50 via-emerald-50 to-teal-50 rounded-2xl p-8 sm:p-12 shadow-lg mx-2 sm:mx-0" style={{ height: containerHeight, minWidth: isMobile ? '90vw' : 'auto' }}>
          <div className="relative">
            <div className="w-12 sm:w-16 h-12 sm:h-16 border-4 border-green-200 border-t-green-600 rounded-full animate-spin"></div>
            <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
              <div className="w-4 sm:w-6 h-4 sm:h-6 bg-green-600 rounded-full animate-pulse"></div>
            </div>
          </div>
          <div className="text-center mt-6">
            <div className="text-lg sm:text-xl font-semibold text-green-800 mb-2">Memuat Presentasi Pengelolaan Sampah...</div>
            <div className="text-sm text-green-600 opacity-80">Mohon tunggu sebentar</div>
            <div className="mt-4 w-32 sm:w-48 h-2 bg-green-200 rounded-full overflow-hidden">
      ...
      {!isLoading && numPages > 0 && (
        <>
          ...
          <div className="w-full max-w-4xl">
            <div className={`flex items-center justify-between bg-gradient-to-r from-green-50 to-emerald-50 p-3 sm:p-4 rounded-2xl shadow-lg border border-green-200 ${isMobile ? 'gap-2' : 'gap-4'}`}>
              ...
              {/* Page Selector */}
              <div className="flex items-center gap-1 sm:gap-3 bg-white rounded-xl px-2 sm:px-4 py-2 sm:py-3 shadow-md border border-green-300 flex-shrink-0">
                <span className="text-xs sm:text-sm font-medium text-gray-700 hidden sm:inline">Slide:</span>
                <select
                  value={currentPage}
                  onChange={(e) => goToPage(Number(e.target.value))}
                  className="bg-transparent border-none outline-none font-medium text-green-700 cursor-pointer text-xs sm:text-sm min-w-[40px] sm:min-w-[60px] focus:ring-2 focus:ring-green-400 rounded-lg px-1 sm:px-2 py-1"
                >
            ...
            {!isMobile && (
              <div className="flex flex-wrap gap-3 justify-center mt-4">
                <button 
                  onClick={() => goToPage(1)}
                  className="px-4 py-2 bg-white hover:bg-green-50 text-green-600 border border-green-200 rounded-lg text-sm font-medium transition-all duration-200 hover:border-green-400 hover:shadow-md"
                >
                  Slide Pertama
                </button>
                <button 
